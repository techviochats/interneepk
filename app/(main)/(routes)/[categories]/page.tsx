...
const CategoriesPage = ({ params }: { params: { categories: string } }) => {
  ...
  if (params.categories === "websitedesign") {
    ...
    data = graphic;
  } else if (params.categories === "appdevelopment") {
      ...
      <div className="flex flex-col gap-y-8 pt-6">
        {data?.map((cur) => (
          <div
            ...
            key={cur?.id}
          >
            <div className="flex gap-x-5 items-center flex-col sm:flex-row gap-y-3">
              {/* logo */}
              <div className="w-20 h-20 p-4 rounded-full flex items-center justify-center bg-white shadow-2xl relative">
                <img
                  src="/assets/htmlcssjs-overview.png"
                  alt=""
                  className="object-cover rounded-full"
                />
              </div>
              <div className="font-semibold text-[32px] flex flex-col flex-1">
